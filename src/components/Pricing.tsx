    ...
    <section className="border-y border-slate-200 bg-[#f6f3ec] py-16 sm:py-24" id="pricing">
      <div className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
        ...
        <div className="mt-10 overflow-hidden rounded-3xl border border-slate-300 bg-white">
          {startingPrices.filter(([name]) => !name.includes('기업 AI 교육')).map(([name, price]) => <div key={name} className="grid gap-2 border-b border-slate-200 px-5 py-4 last:border-0 sm:grid-cols-[1fr_auto] sm:items-center sm:px-7"><span className="text-sm font-semibold text-brand-navy">{name}</span><strong className="text-sm text-brand-blue sm:text-right">{price}</strong></div>)}
        </div>
